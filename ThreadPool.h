 ...
 * @LastEditors: ljy
 * @LastEditTime: 2023-05-16 12:39:54
...
public:
    ...
    ThreadPool(uint primary_num, uint max_secondary_num): 
        ...
        threads_num_(std::make_shared<uint64_t>(primary_num)),
        ...
        num_(0) {
            ...
            for(int i = 0; i < primary_num; i++) {
                primary_threads_->push_back(std::make_shared<PrimaryThread>(tasks_, pool_mutex_ptr_, con_var_ptr_, stop_, num_++));
            }
        }
    ...
    void AddSecondaryThreads(uint num) {
        for (int i = 0; i < num; i++) {
            secondary_threads_->emplace_back(std::make_shared<SecondaryThread>(tasks_, pool_mutex_ptr_, con_var_ptr_, stop_, num_++));
        }
    }
